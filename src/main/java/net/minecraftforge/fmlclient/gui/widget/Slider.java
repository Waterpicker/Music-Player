...
public class Slider extends ExtendedButton
{
    ...
    public Slider(int xPos, int yPos, int width, int height, Text prefix, Text suf, double minVal, double maxVal, double currentVal, boolean showDec, boolean drawStr, PressAction handler, @Nullable ISlider par)
    {
    ...
    protected void renderBackground(MatrixStack mStack, MinecraftClient par1Minecraft, int par2, int par3)
    {
        if (this.visible)
        {
            ...
            GuiUtils.drawContinuousTexturedBox(mStack, WIDGETS_TEXTURE, this.x + (int)(this.sliderValue * (float)(this.width - 8)), this.y, 0, 66, 8, this.height, 200, 20, 2, 3, 2, 2, this.getZOffset());
        }
    }
