...
public class ProgressBar implements Element, Drawable {
	...
	public void render(MatrixStack poseStack, int mouseX, int mouseY, float partialTicks) {
		if (visible) {
			...
			DrawableHelper.fill(poseStack, x, y, x + width, y + height, backgroundColor.getColorARGB());
			DrawableHelper.fill(poseStack, x, y, (int) (x + (progress.get() * width)), y + height, progressColor.getColorARGB());
		}
	}
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button) {
		if (button == 0) {
			if (isPressable(mouseX, mouseY)) {
				playPressSound(MinecraftClient.getInstance().getSoundManager());
